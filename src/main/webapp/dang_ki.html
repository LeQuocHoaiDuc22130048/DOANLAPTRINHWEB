      ...
      <div class="sign-in-form">
        ...
        <form id="register-form">
          ...
          <div class="text-center">
            <span
              >Bạn đã có tài khoản?
              <a href="dang_nhap.html">Đăng nhập ngay</a></span
            >
          </div>
        </form>
      </div>
    ...
    <script>
      $(document).ready(function () {
        ...
        $(".btn-sign-in").click(function (e) {
          ...
          const isUsernameExist = userList.some(
            (user) => user.username === username
          );
          const isEmailExist = userList.some((user) => user.email === email);
          const isPassword = userList.some(
            (user) => user.password === password
          );

          if (isEmailExist && isUsernameExist && isPassword) {
            showAccountExist(
              "Tài khoản đã được đăng kí. Vui lòng chuyển sang trang đăng nhập"
            );
            return;
          }
        ...
        function showAccountExist(message) {
          $("#account-exists-message").text(message).fadeIn();

          setTimeout(() => {
            $("#account-exists-message").fadeOut();
          }, 3000);
        }
      });
    </script>
